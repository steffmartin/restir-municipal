...
import br.com.smaconsulting.sped.editor.task.GenericTask;
import br.com.smaconsulting.sped.editor.task.exportreport.subtask.ReportSubtask;
...
import static br.com.smaconsulting.sped.editor.util.Util.*;
...
public class ExportReportTask extends GenericTask {
    ...
    protected Map<String, Throwable> call() throws Exception {
        ...
        JasperReport report = loadReportTemplate(reportName);
        ...
        updateProgress(1, 1);
        updateMessage(
                hasPages ? "Processo finalizado! Tempo gasto: " + toHMS(watch.getTotalTimeMillis())
    ...
    private String generateFileName() {
        return String.join(" - ",
                ...
                toMY((Timestamp) getFirst(params, LocalDate.now(), "DT_VENDA_INI", "DT_EMI_INI",
                        "DT_ES_INI", "INV1.DT_INV", "DT_INV")) + " a " +
                        toMY((Timestamp) getFirst(params, LocalDate.now(), "DT_VENDA_FIN", "DT_EMI_FIN",
